...
public class ImdbParser {
    ...
    public List<String> getActors(Document doc) {
        ...
        Elements select = doc.select("td[itemProp=\"actor\"] span");
    ...
    public Movie parseMovie(String url) throws IOException {
        ...
        movie.metascore = Double.parseDouble(doc.select(".titleReviewBarItem:first-child span").get(0).text());
        movie.director = doc.select("span[itemProp=\"director\"]").get(0).text();
